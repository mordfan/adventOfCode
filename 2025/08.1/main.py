...
def main(file_name: str, maxNumberOfConnections: int) -> None:
    ...
    for c in range(maxNumberOfConnections):
        if not distances:
            break
    ...
    for k, v in sorted(results.items(), key=lambda i: len(i[1]), reverse=True):
        print(f"{k=}, {v=}, ({len(v)=})")

    print(f"total={reduce((lambda x, y: x * y), sorted(set([len(i) for i in results.values()]), reverse=True)[:3]):,}")
